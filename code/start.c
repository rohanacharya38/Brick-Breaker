...
void loadAudio(void);
void play_break_animation(SDL_Rect *brick);
...
bool Initialize(void)
{
    ...
    char *buf=malloc(sizeof(*buf)*(len+1));
...
void UpdateBricks(void)
{
    ...
    for (int i = 0; i < 20; i++)
    {
        ...
        if (render)
        {
            ...
            // l = i >= 10 && i < 15 ? -255 : 0;
            // m = i >= 10 ? -255 : 0;
            int time=SDL_GetTicks()%3;
            k=(int)(fabs(sin(time))*255);
            l=(int)(fabs(cos(time))*255);
            m=255-k;
            printf("%d,%d,%d\n",k,l,m);
            SDL_SetRenderDrawColor(renderer,  k, l, m, 255);
...

{
    ...
    SDL_Texture *mTexture = SDL_CreateTextureFromSurface(renderer, textSurface);
    gFont = TTF_OpenFont("../misc/arial.ttf", 18);
    SDL_RenderClear(renderer);
    SDL_Rect dest = {.x = WIDTH / 2 - (textSurface->w) / 2, .y = HEIGHT / 2, .w = textSurface->w, .h = textSurface->h};
    SDL_RenderFillRect(renderer, &dest);
    SDL_RenderCopy(renderer, mTexture, NULL, &dest);
...
void loadAudio(void)
{ 
    collision=Mix_LoadWAV("../misc/1.wav");
    if (collision==NULL)
    {
    ...
    bgMusic=Mix_LoadMUS("../misc/2.wav");
    if (collision==NULL)
    {
